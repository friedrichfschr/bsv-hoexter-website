...
export function ArticleEditor() {
  ...
  if (busy) return <p>Artikel werden geladen …</p>;

  return <div className="editorial-layout">
    ...
    <form className="editorial-form editorial-editor" onSubmit={saveArticle}>
      ...
      <div className="editorial-form-grid">
        <div><label htmlFor="article-date">Veröffentlichungsdatum</label><input id="article-date" type="date" value={values.publishedAt} onChange={(event) => updateField("publishedAt", event.target.value)} required /></div>
        <div><label htmlFor="article-status">Status</label><select id="article-status" value={values.status} onChange={(event) => updateField("status", event.target.value as EditorValues["status"])}><option value="draft">Entwurf</option><option value="published">Veröffentlicht</option></select></div>
      </div>
