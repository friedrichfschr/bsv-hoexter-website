...
import { updateEventEntry } from "@/lib/notice-board-moderation";
...
export async function PUT(request: Request, { params }: Context) {
  ...
  try {
    ...
    const bounded = await bufferRequestBody(request, 20_000);
    return NextResponse.json({ event: await updateEventEntry(undefined, id, await bounded.json()) });
  } catch (error) {
    if (error instanceof RequestBodyTooLargeError) return NextResponse.json({ error: "Die Eingabe ist zu groß." }, { status: 413 });
    return NextResponse.json({ error: error instanceof Error ? error.message : "Veranstaltung konnte nicht gespeichert werden." }, { status: 400 });
  }
}